...
get_cis_genotype <- function(gt_df, snp_annot, coords, cis_window) {
  ...
  if (nrow(snp_info) == 0)
    return(NA)
  suppressWarnings(cis_gt <- gt_df %>% select(one_of(snp_info$varID)))
  if (ncol(cis_gt) == 0)
    return(NA)
...
main <- function(snp_annot_file, gene_annot_file, genotype_file, expression_file,
  ...
  for (i in 1:n_genes) {
    ...
    if (ncol(cis_gt) >= 2) {
      ...
      fit <- tryCatch(cv.glmnet(cis_gt, adj_expression, nfolds = n_folds, alpha = alpha, type.measure='mse', foldid = cv_fold_ids, keep = TRUE),
